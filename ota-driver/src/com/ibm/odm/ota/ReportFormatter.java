...
public class ReportFormatter implements Comparator<String> {
	...
	private static Logger logger = Logger.getLogger(ReportFormatter.class.getCanonicalName());
	...
	public void createOutput(Report report, String filename) throws OTAException {
		createHTML(report, filename);
		createJSON(report, filename);
	}

	public void createJSON(Report report, String filename) throws OTAException {
		try {
			String jsonFilename = filename + ".json";
			os = new PrintWriter(new FileWriter(jsonFilename));
			int issues = report.getElements().size();
			os.println("{ \"issues\": " + issues + "}");
			os.close();
		}
		catch ( IOException e ) {
			throw new OTAException("Error generating JSON report", e);
		}
	}
	...
	public void createHTML(Report report, String filename) throws OTAException {
		try {
			...
			os = new PrintWriter(new FileWriter(htmlFilename));
			os.print(template);
			os.close();
	...
	private String getDetails(Report report) throws OTAException {
		...
		try {
			for (ReportElement element : report.getElements()) {
				if (isReportable(element, Context.detail)) {
					...
					if (element.getElement() != null) {
						IlrElementHandle projectHandle = IlrSessionHelper.getProjectNamed(DCConnection.getSession(),
								element.getWhere());
						IlrElementHandle branchHandle = IlrSessionHelper.getBaselineNamed(DCConnection.getSession(),
								projectHandle, element.getBranch());
						String url = permalinkHelper.getElementDetailsURL(element.getWhere(), branchHandle, element.getElement());
			...
			throw new OTAException("Project not found", e);
		}
